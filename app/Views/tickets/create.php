...
<section class="section-padding">
    <div class="mb-4">
        <div class="d-flex align-items-center gap-2 mb-2">
            <a href="<?= site_url('student/dashboard') ?>" class="text-decoration-none" style="color: var(--fu-on-surface-variant);">
            ...
            <div class="card-fu">
                <div class="p-4">
                    ...
                    <div class="mb-4">
                        ...
                        <?= form_dropdown('office_id', $officeOptions, set_value('office_id'), ['class' => 'form-select', 'id' => 'office_id', 'style' => 'border-radius: 8px; border-color: var(--fu-outline-variant);']) ?>
                        <div class="form-text" style="color: var(--fu-error);"><?= isset($validation) ? $validation->getError('office_id') : '' ?></div>
                    </div>

                    <div class="mb-4">
                        <label for="subject" class="form-label fw-semibold">Subject</label>
                        <?= form_input('subject', set_value('subject'), ['class' => 'form-control', 'id' => 'subject', 'placeholder' => 'Briefly describe your issue', 'style' => 'border-radius: 8px; border-color: var(--fu-outline-variant);']) ?>
                        <div class="form-text" style="color: var(--fu-error);"><?= isset($validation) ? $validation->getError('subject') : '' ?></div>
                    </div>

                    <div class="mb-4">
                        <label for="description" class="form-label fw-semibold">Description</label>
                        <?= form_textarea('description', set_value('description'), ['class' => 'form-control', 'id' => 'description', 'rows' => 6, 'placeholder' => 'Provide detailed information about your issue...', 'style' => 'border-radius: 8px; border-color: var(--fu-outline-variant);']) ?>
                        <div class="form-text" style="color: var(--fu-error);"><?= isset($validation) ? $validation->getError('description') : '' ?></div>
                    </div>

                    <div class="mb-4">
                        <label for="priority" class="form-label fw-semibold">Priority</label>
                        <?= form_dropdown('priority', ['Low' => 'Low', 'Medium' => 'Medium', 'High' => 'High', 'Urgent' => 'Urgent'], set_value('priority', 'Medium'), ['class' => 'form-select', 'id' => 'priority', 'style' => 'border-radius: 8px; border-color: var(--fu-outline-variant);']) ?>
